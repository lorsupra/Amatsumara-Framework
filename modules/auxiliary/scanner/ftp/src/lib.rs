...
static NAME: &str = "FTP Anonymous Login Scanner";
static DESCRIPTION: &str = "Tests FTP servers for anonymous login access";
...
static RPORT_DESC: &str = "FTP port";
static RPORT_DEFAULT: &str = "21";
...
impl ScannerModule {
    ...
    async fn test_anonymous(&self) -> Result<String, String> {
        ...
        match tokio::time::timeout(
            ...
            Ok(Ok(_)) => {
                if !banner.starts_with("220") {
                    return Err("Invalid FTP banner".to_string());
                }
            }
            _ => return Err("Failed to read banner".to_string()),
        }
        ...
        // Send USER anonymous
        match stream.write_all(b"USER anonymous\r\n").await {
            Ok(_) => {}
            Err(e) => return Err(format!("Failed to send USER: {}", e)),
        }
        ...
        match tokio::time::timeout(
            ...
            Ok(Ok(_)) => {
                if !response.starts_with("331") {
                    return Err("Anonymous user not accepted".to_string());
                }
            }
            _ => return Err("No response to USER command".to_string()),
        }
        ...
        match stream.write_all(b"PASS anonymous\r\n").await {
            Ok(_) => {}
            Err(e) => return Err(format!("Failed to send PASS: {}", e)),
        }
        ...
        match tokio::time::timeout(
            ...
            Ok(Ok(_)) => {
                if response.starts_with("230") {
                    Ok("Anonymous login successful".to_string())
                } else {
                    Err("Anonymous login failed".to_string())
                }
            }
            _ => Err("No response to PASS command".to_string()),
        }
    }

    async fn run_scan(&self) -> i32 {
        eprintln!("[*] FTP Anonymous Login Scanner");
        eprintln!("[*] Target: {}:{}", self.rhost, self.rport);
        eprintln!();

        match self.test_anonymous().await {
            Ok(msg) => {
                eprintln!("[+] {}: {}", self.rhost, msg);
                0
            }
            Err(e) => {
