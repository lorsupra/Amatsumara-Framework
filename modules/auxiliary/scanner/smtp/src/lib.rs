...
static NAME: &str = "SMTP Banner Scanner";
static DESCRIPTION: &str = "Scans for SMTP servers and retrieves banners";
...
static RPORT_DESC: &str = "SMTP port";
static RPORT_DEFAULT: &str = "25";
...
impl ScannerModule {
    ...
    async fn scan_smtp(&self) -> Result<String, String> {
    ...
    async fn run_scan(&self) -> i32 {
        eprintln!("[*] SMTP Banner Scanner");
        eprintln!("[*] Target: {}:{}", self.rhost, self.rport);
        eprintln!();

        match self.scan_smtp().await {
            Ok(banner) => {
