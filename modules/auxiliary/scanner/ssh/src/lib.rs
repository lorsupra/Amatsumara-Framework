...
static NAME: &str = "SSH Version Scanner";
static DESCRIPTION: &str = "Detects SSH server version and banner information";
...
static RPORT_DESC: &str = "SSH port";
static RPORT_DEFAULT: &str = "22";
...
impl ScannerModule {
    ...
    async fn probe_ssh(&self) -> Result<String, String> {
        ...
        match tokio::time::timeout(
            ...
            Ok(Ok(n)) if n > 0 => {
                ...
                let banner_str = banner_str.trim();
                if banner_str.starts_with("SSH-") {
                    Ok(banner_str.to_string())
                } else {
                    Err("Invalid SSH banner".to_string())
                }
            }
            Ok(Ok(_)) => Err("No banner received".to_string()),
    ...
    async fn run_scan(&self) -> i32 {
        eprintln!("[*] SSH Version Scanner");
        eprintln!("[*] Target: {}:{}", self.rhost, self.rport);
        eprintln!();

        match self.probe_ssh().await {
            Ok(banner) => {
                eprintln!("[+] {}: {}", self.rhost, banner);
                0
            }
            Err(e) => {
