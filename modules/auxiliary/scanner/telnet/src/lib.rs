...
static NAME: &str = "Telnet Banner Scanner";
static DESCRIPTION: &str = "Scans for Telnet servers and retrieves banners";
...
static RPORT_DESC: &str = "Telnet port";
...
impl ScannerModule {
    ...
    async fn run_scan(&self) -> i32 {
        eprintln!("[*] Telnet Banner Scanner");
...
extern "C" fn destroy(instance: *mut c_void) {
    if !instance.is_null() {
        unsafe { let _ = Box::from_raw(instance as *mut ScannerModule); }
    }
}
            ...
            for line in json_str.split(',') {
                ...
                if parts.len() == 2 {
                    ...
                    match key {
                        ...
                        "RPORT" => { if let Ok(port) = value.parse::<u16>() { module.rport = port; } }
                        "TIMEOUT" => { if let Ok(timeout) = value.parse::<u64>() { module.timeout = timeout; } }
                        _ => {}
                    }
                }
            }
