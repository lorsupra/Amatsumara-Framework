///! Multi Handler - Generic payload handler
///!
///! Listens for incoming connections from reverse payloads and creates sessions.
///! Generic reverse payload handler, categorized as a utility.

use amatsumara_api::*;
use amatsumara_api::session_channel::register_session;
use std::os::raw::{c_char, c_int, c_void};
use std::ffi::CStr;
use std::net::{TcpListener, TcpStream};
use std::time::Duration;
use std::io::{Read, Write};
use serde::Deserialize;

// Module metadata
static NAME: &str = "Multi Handler";
static DESCRIPTION: &str = "Generic handler for reverse payloads. Listens for incoming connections and creates sessions.";
static AUTHOR: &str = "Amatsumara Project";
static PLATFORMS: &[Platform] = &[Platform::Multi];
static ARCHS: &[Arch] = &[Arch::X64, Arch::X86, Arch::ARM64];

// Option strings
static LHOST_NAME: &str = "LHOST";
static LHOST_DESC: &str = "The listen address (0.0.0.0 for all interfaces)";
static LHOST_DEFAULT: &str = "0.0.0.0";

static LPORT_NAME: &str = "LPORT";
static LPORT_DESC: &str = "The listen port";
static LPORT_DEFAULT: &str = "4444";

static TIMEOUT_NAME: &str = "TIMEOUT";
static TIMEOUT_DESC: &str = "Seconds to wait for connection (0 = wait forever)";
static TIMEOUT_DEFAULT: &str = "0";

static OPTIONS: &[ModuleOption] = &[
    ModuleOption {
        name: CString { ptr: LHOST_NAME.as_ptr() as *const c_char, len: LHOST_NAME.len() },
        description: CString { ptr: LHOST_DESC.as_ptr() as *const c_char, len: LHOST_DESC.len() },
        required: true,
        option_type: OptionType::Address,
        default_value: CString { ptr: LHOST_DEFAULT.as_ptr() as *const c_char, len: LHOST_DEFAULT.len() },
    },
    ModuleOption {
        name: CString { ptr: LPORT_NAME.as_ptr() as *const c_char, len: LPORT_NAME.len() },
        description: CString { ptr: LPORT_DESC.as_ptr() as *const c_char, len: LPORT_DESC.len() },
        required: true,
        option_type: OptionType::Port,
        default_value: CString { ptr: LPORT_DEFAULT.as_ptr() as *const c_char, len: LPORT_DEFAULT.len() },
    },
    ModuleOption {
        name: CString { ptr: TIMEOUT_NAME.as_ptr() as *const c_char, len: TIMEOUT_NAME.len() },
        description: CString { ptr: TIMEOUT_DESC.as_ptr() as *const c_char, len: TIMEOUT_DESC.len() },
        required: false,
        option_type: OptionType::Int,
        default_value: CString { ptr: TIMEOUT_DEFAULT.as_ptr() as *const c_char, len: TIMEOUT_DEFAULT.len() },
    },
];

static MODULE_INFO: ModuleInfo = ModuleInfo {
    api_version: MODULE_API_VERSION,
    metadata: ModuleMetadata {
        name: CString { ptr: NAME.as_ptr() as *const c_char, len: NAME.len() },
        description: CString { ptr: DESCRIPTION.as_ptr() as *const c_char, len: DESCRIPTION.len() },
        author: CString { ptr: AUTHOR.as_ptr() as *const c_char, len: AUTHOR.len() },
        module_type: ModuleType::Utility,
        platforms: PlatformArray { ptr: PLATFORMS.as_ptr(), len: PLATFORMS.len() },
        archs: ArchArray { ptr: ARCHS.as_ptr(), len: ARCHS.len() },
        ranking: Ranking::Excellent,
        privileged: false,
    },
    options: OptionArray { ptr: OPTIONS.as_ptr(), len: OPTIONS.len() },
};

static VTABLE: ModuleVTable = ModuleVTable {
    get_info,
    init,
    destroy,
    check: None,
    run,
};

struct ModuleInstance {
    _dummy: u8,
}

#[derive(Deserialize, Debug)]
struct HandlerOptions {
    #[serde(rename = "LHOST")]
    lhost: Option<String>,
    #[serde(rename = "LPORT")]
    lport: Option<String>,
    #[serde(rename = "TIMEOUT")]
    timeout: Option<String>,
}

extern "C" fn get_info() -> *const ModuleInfo {
    &MODULE_INFO
}

extern "C" fn init() -> *mut c_void {
    Box::into_raw(Box::new(ModuleInstance { _dummy: 0 })) as *mut c_void
}

extern "C" fn destroy(instance: *mut c_void) {
    if !instance.is_null() {
        unsafe {
            let _ = Box::from_raw(instance as *mut ModuleInstance);
        }
    }
}

extern "C" fn run(_instance: *mut c_void, config: *const c_char) -> c_int {
    let options = match parse_options(config) {
        Ok(opts) => opts,
        Err(e) => {
            eprintln!("[-] Failed to parse options: {}", e);
            return 1;
        }
    };

    let lhost = options.lhost.unwrap_or_else(|| "0.0.0.0".to_string());
    let lport: u16 = options.lport
        .and_then(|p| p.parse().ok())
        .unwrap_or(4444);
    let timeout: u64 = options.timeout
        .and_then(|t| t.parse().ok())
        .unwrap_or(0);

    let bind_addr = format!("{}:{}", lhost, lport);

    eprintln!("[*] Starting handler on {}", bind_addr);

    // Bind the listener
    let listener = match TcpListener::bind(&bind_addr) {
        Ok(l) => l,
        Err(e) => {
            eprintln!("[-] Failed to bind to {}: {}", bind_addr, e);
            return 1;
        }
    };

    eprintln!("[*] Waiting for connections...");

    // Set timeout if specified
    if timeout > 0 {
        listener.set_nonblocking(false).ok();
        // For timeout, we'll use a simple approach
        let timeout_duration = Duration::from_secs(timeout);
        let start = std::time::Instant::now();

        listener.set_nonblocking(true).ok();

        loop {
            match listener.accept() {
                Ok((stream, addr)) => {
                    return handle_connection(stream, addr.ip().to_string(), addr.port());
                }
                Err(ref e) if e.kind() == std::io::ErrorKind::WouldBlock => {
                    if start.elapsed() >= timeout_duration {
                        eprintln!("[-] Timeout waiting for connection");
                        return 1;
                    }
                    std::thread::sleep(Duration::from_millis(100));
                }
                Err(e) => {
                    eprintln!("[-] Accept error: {}", e);
                    return 1;
                }
            }
        }
    } else {
        // Wait forever for a connection
        match listener.accept() {
            Ok((stream, addr)) => {
                handle_connection(stream, addr.ip().to_string(), addr.port())
            }
            Err(e) => {
                eprintln!("[-] Accept error: {}", e);
                1
            }
        }
    }
}

fn handle_connection(mut stream: TcpStream, remote_host: String, remote_port: u16) -> c_int {
    eprintln!("[+] Connection received from {}:{}", remote_host, remote_port);

    // Set timeouts for the stream
    stream.set_read_timeout(Some(Duration::from_secs(5))).ok();
    stream.set_write_timeout(Some(Duration::from_secs(5))).ok();

    // Try to verify it's a shell by sending a test command
    eprintln!("[*] Verifying shell access...");

    // Send id command to verify
    if stream.write_all(b"id\n").is_err() {
        eprintln!("[-] Failed to send verification command");
        // Still register the session - might be a different type of shell
    }

    // Give it a moment to respond
    std::thread::sleep(Duration::from_millis(500));

    // Try to read response
    let mut buf = [0u8; 1024];
    match stream.read(&mut buf) {
        Ok(n) if n > 0 => {
            let response = String::from_utf8_lossy(&buf[..n]);
            if response.contains("uid=") {
                eprintln!("[+] Shell verified: {}", response.trim());
            } else {
                eprintln!("[*] Response: {}", response.trim());
            }
        }
        Ok(_) => {
            eprintln!("[*] No immediate response (shell may still be valid)");
        }
        Err(_) => {
            eprintln!("[*] No immediate response (shell may still be valid)");
        }
    }

    // Clear timeouts for interactive use
    stream.set_read_timeout(None).ok();
    stream.set_write_timeout(None).ok();

    // Register the session with the framework
    let description = format!("Shell from {}:{}", remote_host, remote_port);
    register_session(stream, remote_host, remote_port, description);

    eprintln!("[+] Session created");
    eprintln!("[*] Use 'sessions -l' to list and 'sessions -i <id>' to interact");

    0
}

fn parse_options(config: *const c_char) -> Result<HandlerOptions, Box<dyn std::error::Error>> {
    if config.is_null() {
        return Ok(HandlerOptions {
            lhost: None,
            lport: None,
            timeout: None,
        });
    }

    let config_str = unsafe { CStr::from_ptr(config).to_str()? };
    let options: HandlerOptions = serde_json::from_str(config_str)?;
    Ok(options)
}

#[no_mangle]
pub extern "C" fn amatsumara_module_init() -> *const ModuleVTable {
    &VTABLE
}
